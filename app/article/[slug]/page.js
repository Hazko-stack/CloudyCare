...
async function getArticleBySlug(slug) {
  ...
  const matchingFile = allArticles.find(filename => {
  ...
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
  const url = `${supabaseUrl}/storage/v1/object/public/health-articles/mdx/${matchingFile}`
  ...
  try {
    ...
    return {
      ...
      frontmatter: {
        ...
        coverImage: getSupabaseImageUrl(frontmatter.coverImage),
